...
namespace WhatIsFunction
{
    public class MineGame
    {
        public static void Main()
        {
             ...
             * debug 모드에서 지뢰가 아닌 곳은 .(닷) 지뢰인 곳은 #(샵)으로 표현
             * play 모드에서 확인되지 않는 곳은 전부 □(스퀘어)로 표현
             * 첫 턴에 지뢰를 밟으면 해당 칸에 지뢰를 지워줌
             */
            ...
            while (!isGameOver)
            {
                    ...
                    for (int x = 0; x < BOARD_SIZE_X; x++)
                    {
                        switch (playBoard[y, x])
                        {
                            ...
                            case 0:
                                ...
                                break;
                            default:
                                Console.Write($"{playBoard[y, x]}".PadRight(3, ' '));
                                break;
                        } // switch
                    } // loop
                    Console.WriteLine();
                ...
                while (!isLocationValid)
                {
                    ...
                    // 플레이어가 입력한 좌표의 유효성 검사
                    isLocationValid =
                        (0 <= playerX && playerX < BOARD_SIZE_X) &&
                        (0 <= playerY && playerY < BOARD_SIZE_Y);
                    if (!isLocationValid)
                    {
                        Console.WriteLine("해당 좌표는 유효하지 않습니다. 다른 좌표를 입력하세요.\n");
                        continue;
                    }

                    // 좌표를 제대로 입력한 경우만 이 아래로 코드가 진행.
                    // 유효하지 않은 경우 위에서 컨티뉴 실행

                    // 플레이 보드에서 선택 가능한지 검사
                    isLocationValid = isLocationValid && playBoard[playerY, playerX].Equals(-1);
                    if (!isLocationValid)
                    {
                        Console.WriteLine("해당 좌표는 이미 오픈 되었습니다. 다른 좌표를 입력하세요.\n");
                        continue;
                    } // if : 오픈된 좌표를 선택한 경우
                ...
                for (int y = 0; y < BOARD_SIZE_Y; y++)
                {
                    ...
                    for (int x = 0; x < BOARD_SIZE_X; x++)
                    {
                        ...
                        {
                            unknownTileCnt++;
                        }       // if : 아직 오픈할 타일이 존재 + 해당 타일이 지뢰가 아닌 경우
                    }
